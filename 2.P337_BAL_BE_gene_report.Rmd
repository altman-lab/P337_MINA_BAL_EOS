...
```{r echo=FALSE}
summarise_lmFit(lm_visit_BAL$lm, fdr.cutoff = c(0.05,0.1,0.3)) %>% 
  mutate(sample = "BAL", .before = variable) %>% 
  bind_rows(summarise_lmFit(lm_visit_BE$lm, fdr.cutoff = c(0.05,0.1,0.3)) %>% 
...
```{r echo=FALSE}
summarise_lmFit(lm_eos_BAL$lm, fdr.cutoff = c(0.05,0.1,0.3)) %>% 
  mutate(sample = "BAL", .before = variable) %>% 
  bind_rows(summarise_lmFit(lm_eos_BE$lm, fdr.cutoff = c(0.05,0.1,0.3)) %>% 
              mutate(sample = "BE", .before = variable)) %>% 
  rename_all(~gsub("_", " < ", .)) %>% 
...
```{r echo=FALSE}
summarise_lmFit(lm_feno_BAL$lm, fdr.cutoff = c(0.05,0.1,0.3)) %>% 
  mutate(sample = "BAL", .before = variable) %>% 
  bind_rows(summarise_lmFit(lm_feno_BE$lm, fdr.cutoff = c(0.05,0.1,0.3)) %>% 
              mutate(sample = "BE", .before = variable)) %>% 
  rename_all(~gsub("_", " < ", .)) %>% 
...
## Genes significant for eosinophils and/or FeNO

Allergen challenge (FDR < 0.3) and eosinophil significant (FDR 0.1) genes in BAL vs BE. As expected, genes can be grouped into those that positively associate with eosinophils in both BAL and BE (positive correlation) and those that have oppositive directionality in BAL and BE (negative correlation).
...
#FeNO signif genes
BAL_feno_0.1 <- lm_feno_BAL$lm %>% 
  filter(variable == "FeNO" & FDR < 0.1) %>% 
  pull(gene)
BE_feno_0.1 <- lm_feno_BE$lm %>% 
  filter(variable == "FeNO" & FDR < 0.1) %>% 
  pull(gene)
...
temp <- dat.BAL.rename %>% 
  ...
  mutate(EOS = ifelse(gene %in% c(BAL_eos_0.1_up, BAL_eos_0.1_dn), "FDR < 0.1", "NS"),
         FeNO = ifelse(gene %in% BAL_feno_0.1, "FDR < 0.1", "NS")) %>% 
  mutate(gene_label = paste("BAL",gene, sep = "_")) 
...
dat.visit.all.rename <- dat.BE.rename %>% 
  ...
  mutate(EOS = ifelse(gene %in% c(BE_eos_0.1_up, BE_eos_0.1_dn), "FDR < 0.1", "NS"),
         FeNO = ifelse(gene %in% BE_feno_0.1, "FDR < 0.1", "NS")) %>% 
  mutate(gene_label = paste("BE",gene, sep = "_"))  %>% 
  bind_rows(temp) %>% 
  arrange(donorID) %>% 
...
col.anno <- HeatmapAnnotation(
  `BE EOS FDR < 0.1` = filter(hm.mat.up, grepl("BE_", gene_label))$EOS,
  `BE FeNO FDR < 0.1` = filter(hm.mat.up, grepl("BE_", gene_label))$FeNO,
  col = list(`BE EOS FDR < 0.1` = c("FDR < 0.1" = "#E1BE6A", "NS" = "white"),
             `BE FeNO FDR < 0.1` = c("FDR < 0.1" = "#40B0A6", "NS" = "white")), 
        show_legend = c(FALSE,FALSE))
row.anno <- rowAnnotation(
  `BAL EOS FDR < 0.1` = filter(hm.mat.up, grepl("BAL_", gene_label))$EOS,
  `BAL FeNO FDR < 0.1` = filter(hm.mat.up, grepl("BAL_", gene_label))$FeNO,
...
col.anno2 <- HeatmapAnnotation(
  `BE EOS FDR < 0.1` = filter(hm.mat.dn, grepl("BE_", gene_label))$EOS,
  `BE FeNO FDR < 0.1` = filter(hm.mat.dn, grepl("BE_", gene_label))$FeNO,
  col = list(`BE EOS FDR < 0.1` = c("FDR < 0.1" = "#E1BE6A", "NS" = "white"),
             `BE FeNO FDR < 0.1` = c("FDR < 0.1" = "#40B0A6", "NS" = "white")), 
        show_legend = c(FALSE,FALSE))
row.anno2 <- rowAnnotation(
  `BAL EOS FDR < 0.1` = filter(hm.mat.dn, grepl("BAL_", gene_label))$EOS,
  `BAL FeNO FDR < 0.1` = filter(hm.mat.dn, grepl("BAL_", gene_label))$FeNO,
  col = list(`BAL EOS FDR < 0.1` = c("FDR < 0.1" = "#E1BE6A", "NS" = "white"),
             `BAL FeNO FDR < 0.1` = c("FDR < 0.1" = "#40B0A6", "NS" = "white")), 
        show_legend = c(FALSE,FALSE))
...
See `correlation_heatmap_clusters.csv` for gene membership in each cluster. The two heatmaps are designated as `eos_up` and `eos_down`

```{r}
#get clusters, rows
cluster.row <- data.frame()
for (i in 1:length(row_order(hm.up))){
  cluster.row <- t(t(row.names(corr.up.R[row_order(hm.up)[[as.character(i)]],]))) %>% 
    as.data.frame() %>% 
    mutate(heatmap = "eos_up",
           cluster = paste("BAL_row_cluster", i, sep="")) %>% 
    rename(gene=V1) %>% 
    bind_rows(cluster.row)
}

for (i in 1:length(row_order(hm.dn))){
  cluster.row <- t(t(row.names(corr.dn.R[row_order(hm.dn)[[as.character(i)]],]))) %>% 
    as.data.frame() %>% 
    mutate(heatmap = "eos_down",
           cluster = paste("BAL_row_cluster", i, sep="")) %>% 
    rename(gene=V1) %>% 
    bind_rows(cluster.row)
}
...
cluster.col <- data.frame()
for (i in 1:length(column_order(hm.up))){
  cluster.col <- t(t(colnames(corr.up.R[,column_order(hm.up)[[as.character(i)]]]))) %>% 
    as.data.frame() %>% 
    mutate(heatmap = "eos_up",
           cluster = paste("BE_column_cluster", i, sep="")) %>% 
    rename(gene=V1) %>% 
    bind_rows(cluster.col)
}

for (i in 1:length(column_order(hm.dn))){
  cluster.col <- t(t(colnames(corr.dn.R[,column_order(hm.dn)[[as.character(i)]]]))) %>% 
    as.data.frame() %>% 
    mutate(heatmap = "eos_down",
           cluster = paste("BE_column_cluster", i, sep="")) %>% 
    rename(gene=V1) %>% 
