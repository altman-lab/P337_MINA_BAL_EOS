...
#### Data ####
# Cell counts
count <- read_csv("data_raw/P337_cell.counts.csv") %>% 
  select(ptID, contains("BAL")) %>% 
  pivot_longer(-ptID) %>% 
  separate(name, into=c("name","visit"), sep="_V") %>% 
  pivot_wider() %>%
  select(-BAL_Epi_pct) %>% 
  mutate(visit = paste0("V",visit)) %>% 
  mutate(across(c(BAL_WBC_no:BAL_MONO_no), ~./BAL_volume.ml)) %>% 
  rename(donorID=ptID)
...
targets.all.format <- targets.all %>% 
  distinct(donorID, visit, BAL_EOS_no, BAL_PMN_no) %>% 
  pivot_longer(-c(donorID, visit), names_to = "variable") %>% 
  mutate(fdr = recode(variable, 
                      "BAL_EOS_no"="eosinophils",
                      "BAL_PMN_no"="neutrophils"),
...
p1 <- targets.all.format %>% 
  filter(variable %in% c("BAL_EOS_no","BAL_PMN_no")) %>% 
  ggplot() +
  aes(x=visit, y= value) +
  ...
  labs(y="Cells per mL", x="Segmented bronchial\nprovocation with allergen\n(SBP-Ag)",
...
dat.eos <- targets.all %>% 
  select(donorID, visit, BAL_EOS_no) %>% 
  mutate(name="BAL_EOS_no") %>% 
  rename(value=BAL_EOS_no)
...
p2 <- full_join(dat.eos,dat.feno) %>% 
  ...
  ggplot(aes(x=BAL_EOS_no,y=FeNO.PreBro)) +
  geom_point(color="grey") +
  theme_classic() +
  stat_cor() +
  geom_smooth(method="lm", se=FALSE, formula = "y~x", color="black") +
  labs(x = "eosinophils per mL", y = "FeNO") +
  facet_wrap(~visit, scales="free", nrow=1) +
  theme(legend.position = "none")
 # p2

dat.neut <- targets.all %>% 
  select(donorID, visit, BAL_PMN_no) %>% 
  mutate(name="BAL_PMN_no") %>% 
  rename(value=BAL_PMN_no)

p3 <- full_join(dat.neut,dat.feno) %>% 
  pivot_wider() %>%
  mutate(visit = fct_recode(visit,"Pre SPB-Ag"="V4","Post SBP-Ag"="V5")) %>% 
  
  ggplot(aes(x=BAL_PMN_no,y=FeNO.PreBro)) +
  geom_point(color="grey") +
  theme_classic() +
  stat_cor() +
  geom_smooth(method="lm", se=FALSE, formula = "y~x", color="black") +
  labs(x = "neutrophils per mL", y = "FeNO") +
  facet_wrap(~visit, scales="free", nrow=1) +
  theme(legend.position = "none")
# p3
...
# plot_all
ggsave(filename = "publication/FigX_cells.feno2.png", plot_all,
       width=8.5, height=7)
